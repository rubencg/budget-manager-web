...
@Component({
  selector: 'expenses-by-subcategory',
  templateUrl: './expenses-by-subcategory.component.html',
  styleUrls: ['./expenses-by-subcategory.component.scss']
})
export class ExpensesBySubcategoryComponent implements OnInit, AfterViewInit {
  ...
  changeDate(date: Date){
    this.expenses$ = this.store.select((state) => state.expenseState.expenses).pipe(
      ...
      map((expenses: Expense[]) => {
        ...
        const expensesByCategory = _.groupBy(appliedExpenses, (e: Expense) => e.subCategory ? e.subCategory : e.category.name);
